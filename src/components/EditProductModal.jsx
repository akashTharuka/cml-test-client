...
const EditProductModal = ({ open, handleOpen, data }) => {
    ...
    // populate the form with the data if it exists
    useEffect(() => {
        setEventId(data?.eventId || '');
        setTransId(data?.transId || '');
        setTransTms(data?.transTms || '');
        setRcNum(data?.rcNum || '');
        setClientId(data?.clientId || '');
        setEventCnt(data?.eventCnt || '');
        setLocationCd(data?.locationCd || '');
        setLocationId1(data?.locationId1 || '');
        setLocationId2(data?.locationId2 || '');
        setAddrNbr(data?.addrNbr || '');
    }, [data]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put('/api/events', payload,
                {
                    headers: {
